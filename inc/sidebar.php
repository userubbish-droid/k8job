...
<aside class="dashboard-sidebar">
    ...
    <?php if (has_permission('customers') || has_permission('customer_create')): ?>
    <div class="nav-group" data-group="account-customer">
        <button type="button" class="nav-group-toggle nav-item" aria-expanded="<?= in_array($sidebar_current, ['customers', 'customer_create'], true) ? 'true' : 'false' ?>" aria-controls="nav-sub-account-customer" id="nav-toggle-account-customer">
            <span class="nav-icon"></span>
            <span class="nav-group-label">Account - Customer</span>
            <span class="nav-group-chevron" aria-hidden="true">▾</span>
        </button>
        <div class="nav-group-sub" id="nav-sub-account-customer" role="region" aria-labelledby="nav-toggle-account-customer" style="display:<?= in_array($sidebar_current, ['customers', 'customer_create'], true) ? 'block' : 'none' ?>">
    ...
    <?php endif; ?>
    <?php if (has_permission('product_library')): ?>
    <div class="nav-group" data-group="product">
        <button type="button" class="nav-group-toggle nav-item" aria-expanded="<?= $sidebar_current === 'product_library' ? 'true' : 'false' ?>" aria-controls="nav-sub-product" id="nav-toggle-product">
            <span class="nav-icon"></span>
            <span class="nav-group-label">Product</span>
            <span class="nav-group-chevron" aria-hidden="true">▾</span>
        </button>
        <div class="nav-group-sub" id="nav-sub-product" role="region" aria-labelledby="nav-toggle-product" style="display:<?= $sidebar_current === 'product_library' ? 'block' : 'none' ?>">
            <a href="product_library.php" class="nav-item nav-sub-item <?= $sidebar_current === 'product_library' ? 'primary' : '' ?>"><span class="nav-icon"></span>Product Accounts</a>
        </div>
    </div>
    <?php endif; ?>
